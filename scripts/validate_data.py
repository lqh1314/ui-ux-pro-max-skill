...
WCAG_CONFORMANCE = re.compile(r"\bWCAG\s+A{2,3}\+?\b|\bWCAG\b.{0,40}\b(?:compliant|compliance)\b", re.I)
WCAG_GRADE = re.compile(r"\bWCAG\s+A{1,3}\b", re.I)
CHART_TEXT_FALLBACK = re.compile(r"table|list|summary|text")
CHART_NON_COLOR_GUIDANCE = re.compile(r"color alone|hue alone|alone is insufficient|only distinction|only carriers?|supplementary|pattern|symbol|outline|direct (?:series |group )?label|marker shape|label every")
CSS_IMPORT = re.compile(r"\s*@import\s+url\((['\"])(https://fonts\.googleapis\.com/css2\?[^'\"]+)\1\);\s*", re.I)
FONT_WEIGHT = re.compile(r"(?<!\d)(?:[1-9]00)(?!\d)")
ICON_IMPORT = re.compile(r"import\s*\{\s*[A-Z][A-Za-z0-9]*(?:\s*,\s*[A-Z][A-Za-z0-9]*)*\s*\}\s*from\s*['\"](?:@phosphor-icons/react|phosphor-react-native|@heroicons/react/24/(?:outline|solid))['\"]")
ICON_USAGE_REQUIREMENTS = (r"aria-hidden", r"text alternative", r"accessible name", r"aria-(?:pressed|expanded)")
LANDING_QUANTIFIED_CLAIM = re.compile(r"\b\d+(?:\.\d+)?x\b|\b(?:increases?|reduces?)\s+(?:engagement|conversion|returns?)\b", re.I)
...
PROVENANCE_APPLIES_TO = {"search", "design-guidance", "design-system", "gallery", "style-search"}
CORE_PROVENANCE_FILES = {"colors.csv", "charts.csv", "ux-guidelines.csv", "landing.csv", "typography.csv", "icons.csv", "motion.csv", "app-interface.csv", "react-performance.csv", "stacks/html-tailwind.csv"}
CATALOG_PROVENANCE_FILES = {"google-fonts.csv", "phosphor-icons-upstream.json"}
CATALOG_PROVENANCE_IDS = {"google-fonts-catalog-2026-08-13", "phosphor-icons-catalog-2.1.1"}
...
OFFICIAL_SOURCE_HOSTS = {"carbondesignsystem.com", "developer.android.com", "developer.apple.com", "developers.google.com", "fluent2.microsoft.design", "github.com", "greensock.com", "gsap.com", "m3.material.io", "opensource.adobe.com", "react.dev", "s2.spectrum.adobe.com", "shopify.dev", "design-system.service.gov.uk", "tailwindcss.com", "www.w3.org"}
STACK_OFFICIAL_HOSTS = {
    "react": {"react.dev"}, "nextjs": {"nextjs.org"}, "vue": {"vuejs.org", "pinia.vuejs.org"},
    "svelte": {"svelte.dev", "kit.svelte.dev"}, "astro": {"docs.astro.build"}, "angular": {"angular.dev"},
    "html-tailwind": {"tailwindcss.com"}, "shadcn": {"ui.shadcn.com"}, "nuxtjs": {"nuxt.com"},
    "nuxt-ui": {"ui.nuxt.com"}, "react-native": {"reactnative.dev", "react.dev"},
    "flutter": {"api.flutter.dev", "docs.flutter.dev"}, "swiftui": {"developer.apple.com"},
    "jetpack-compose": {"developer.android.com"}, "avalonia": {"docs.avaloniaui.net"}, "uwp": {"learn.microsoft.com"},
    "winui": {"learn.microsoft.com"}, "wpf": {"learn.microsoft.com"}, "uno": {"platform.uno"},
    "javafx": {"openjfx.io", "mkpaz.github.io", "www.w3.org"},
    "threejs": {"threejs.org", "github.com", "www.npmjs.com", "www.w3.org"}, "laravel": {"laravel.com"},
}
REQUIRED_UX_GUIDANCE = {
    "Focus Not Obscured (Minimum)": "Web", "Focus Not Obscured (Enhanced)": "Web",
    "Focus Appearance": "Web", "Dragging Movements": "All", "Target Size (Minimum)": "Web",
    "Consistent Help": "All", "Redundant Entry": "All", "Accessible Authentication (Minimum)": "All",
...
def _check_file(label, filepath, search_cols, output_cols, problems):
    ...
    if "No" in header_set:
        ...
        for i, row in enumerate(rows, start=2):
            key = row.get("No", "")
            if key in seen:
                problems.append(f"[{label}] {filepath.name}: duplicate 'No' value '{key}' on rows {seen[key]} and {i}")
            else:
                seen[key] = i
    elif label.startswith("stack:"):
        problems.append(f"[{label}] {filepath.name}: missing 'No' index column present in other stack files (schema drift -- harmless for search, but inconsistent with the rest of data/stacks/)")
    return headers, rows
...
def _valid_confidence(value):
    return value is None or (not isinstance(value, bool) and isinstance(value, (int, float)) and math.isfinite(value) and 0 <= value <= 1)

def _check_style_contract(styles, products, reasoning, problems, replacement_targets=None):
    ...
    for row in styles:
        ...
        if status == "deprecated":
            ...
            if has_parent == has_redirect:
                problems.append(f"[style:{style_id}] deprecated rows require exactly one parent or redirect")
            if has_redirect and replacement_id not in replacement_targets.get(replacement_domain, set()):
                problems.append(f"[style:{style_id}] invalid {replacement_domain} redirect '{replacement_id}'")
    ...
    for reference in sorted(set(references)):
        ...
        elif next(row for row in styles if row.get("Style ID") == resolved_id).get("Status") == "deprecated":
    ...
    for row in styles:
        ...
        if any(term in row.get("Framework Compatibility", "").casefold() for term in ("all frameworks", "excellent", "performant", "lightweight")):
...
def _check_reasoning_contract(products, colors, reasoning, style_ids, patterns, problems):
    semantic_sets = (("products", products, "Product Type"), ("colors", colors, "Product Type"), ("reasoning", reasoning, "UI_Category"))
    for label, rows, key in semantic_sets:
        ...
        if duplicates:
            problems.append(f"[reasoning] duplicate {label} labels: {', '.join(duplicates)}")
    ...
    for row in reasoning:
        ...
        if pattern not in patterns:
            problems.append(f"[reasoning:{category}] unknown Recommended_Pattern '{pattern}'")
    ...
    for row in rows:
        ...
        for foreground, (background, role, minimum) in COLOR_CONTRAST_PAIRS.items():
            ...
            if ratio + 1e-9 < minimum:
                problems.append(f"[color:{product}] {role} {foreground}/{background} contrast {ratio:.2f}:1 is below {minimum:.1f}:1")
            ratios[foreground] = ratio
        if ("Muted Foreground" in ratios and "Foreground" in ratios and ratios["Muted Foreground"] > ratios["Foreground"] + 1e-9):
            problems.append(f"[color:{product}] muted text contrast exceeds primary text contrast")
        try:
...
def _check_chart_contract(rows, problems):
    for row in rows:
        ...
        fallback = " ".join((row.get("Accessibility Notes", ""), row.get("A11y Fallback", ""))).casefold()
...
def _font_names(family_declarations):
    return {declaration.split(":", 1)[0].replace("+", " ") for declaration in family_declarations}
...
def _valid_google_fonts_exclusion_source(value):
    ...
    return parsed.hostname == "github.com" and (parsed.path == "/google/fonts" or parsed.path.startswith("/google/fonts/"))
...
def _check_font_catalog(rows, licenses, typography, problems):
    ...
    if (not isinstance(source, dict) or source.get("repository") != "https://github.com/google/fonts" or source.get("metadataFile") != "METADATA.pb" or not GOOGLE_FONTS_REVISION.fullmatch(source.get("revision", ""))):
    ...
    if (licenses.get("familyCount") != len(rows) or not isinstance(entries, list) or not isinstance(excluded, list)):
    ...
    for item in entries:
        if not isinstance(item, dict):
            ...
            continue
        name = item.get("name")
        if (not isinstance(name, str) or not name or name in licensed_names or item.get("license") not in FONT_LICENSES or item.get("status") != "active" or not _catalog_date(item.get("date_added")) or not _catalog_date(item.get("verifiedAt"))):
    ...
    for item in excluded:
        ...
        if (not isinstance(name, str) or not name or name in excluded_names or name in family_set or item.get("status") != "needs-review" or not isinstance(item.get("reason"), str) or not item.get("reason") or not _valid_google_fonts_exclusion_source(source) or not _catalog_date(item.get("verifiedAt"))):
...
def _check_phosphor_catalog(curated, manifest, problems):
    ...
    if (manifest.get("status") != "active" or not _catalog_date(manifest.get("verifiedAt")) or set(manifest.get("weights", [])) != PHOSPHOR_WEIGHTS or imports != {"clientModule": "@phosphor-icons/react", "ssrModule": "@phosphor-icons/react/ssr"} or not isinstance(icons, list) or manifest.get("iconCount") != len(icons)):
    ...
    for item in icons:
        ...
        if (not name or name in names or not component or component in components or item.get("clientImport") != f'import {{ {component} }} from "@phosphor-icons/react"' or item.get("ssrImport") != f'import {{ {component} }} from "@phosphor-icons/react/ssr"'):
...
def _check_catalog_summary(summary, licenses, phosphor, problems):
    ...
    expected = {
        "styles": {"total": len(styles), "searchable": sum(row.get("Status") != "deprecated" for row in styles), "active": sum(row.get("Status") == "active" for row in styles), "supplemental": sum(row.get("Status") == "supplemental" for row in styles), "deprecated": sum(row.get("Status") == "deprecated" for row in styles)},
    ...
    if policy != {"changedFamilySetRequiresExplicitApproval": True, "relevanceGateRequired": True, "unlicensedFamiliesExcluded": True}:
        problems.append("[catalog:summary] invalid promotion policy")
    pending = sorted(({"family": item.get("name"), "reason": item.get("reason")} for item in licenses.get("excludedFamilies", []) if isinstance(item, dict)), key=lambda item: (item.get("family") or "").casefold())
...
def _check_catalog_contract(domain_rows, problems):
    ...
    if licenses:
        _check_font_catalog(domain_rows.get("google-fonts", []), licenses, domain_rows.get("typography", []), problems)
    if phosphor:
...
def _check_icon_contract(rows, problems):
    for row in rows:
        ...
        if any(not re.search(pattern, usage, re.I) for pattern in ICON_USAGE_REQUIREMENTS):
...
def _check_ux_contract(rows, problems):
    ...
    for issue, platform in REQUIRED_UX_GUIDANCE.items():
        ...
        if row and (row.get("Platform") != platform or row.get("Severity") not in {"Medium", "High", "Critical"}):
...
def _check_app_interface_contract(rows, problems):
    native_target = next((row for row in rows if row.get("Issue") == "Touch Target Size"), {})
...
def _check_react_contract(rows, problems):
    ...
    effect_event = next((row for row in rows if row.get("Issue") == "Effect Events"), {})
...
def _check_landing_claims(rows, problems):
    for row in rows:
        ...
        if ("%" in optimization or LANDING_QUANTIFIED_CLAIM.search(optimization) or "best conversion" in optimization.casefold()):
            problems.append(f"[landing:{row.get('Pattern Name')}] unsupported quantitative claim")
...
def _valid_provenance_source(source, source_index, identity, problems):
    if not isinstance(source, dict):
        problems.append(f"[provenance] source {source_index} for {identity} must be an object")
        return False
    ...
    if source_type == "official":
        ...
        problems.append(f"[provenance] derived source must use a local dataset reference for {identity}")
        return False
    return True
...
def _valid_dataset_source_key(source_file, source_key, identity, problems):
    ...
    if not any(re.search(rf"\b{re.escape(header)}\b", scope, re.I) for header in semantic_headers):
...
def _check_provenance(reasoning, styles, problems):
    ...
    try:
        ...
        return
    if (not isinstance(payload, dict) or payload.get("schemaVersion") != 1 or not isinstance(payload.get("records"), list)):
    ...
    for index, record in enumerate(payload["records"]):
        ...
        if (not isinstance(applies_to, list) or not applies_to or any(value not in PROVENANCE_APPLIES_TO for value in applies_to)):
        ...
        if not isinstance(sources, list) or not sources:
            ...
            sources = []
        valid_sources = [source for source_index, source in enumerate(sources) if _valid_provenance_source(source, source_index, identity, problems)]
        source_types = {source["type"] for source in valid_sources}
        if record.get("sla") == "manual-verified" and source_types <= {"derived"}:
            problems.append(f"[provenance] {identity} cannot be manual-verified from derived sources only")
        ...
        if identity[0] == "dataset-contract":
            source_key_valid = _valid_dataset_source_key(record.get("sourceFile"), source_key, identity, problems)
        elif identity[0] == "catalog-snapshot":
            source_key_valid = _valid_catalog_source_key(record.get("sourceFile"), source_key, identity, problems)
    ...
    if new_styles - covered_styles:
        problems.append(f"[provenance] missing new style rows: {', '.join(sorted(new_styles - covered_styles))}")
    core_files = {record.get("sourceFile") for record in valid_records if record.get("entityKind") == "dataset-contract" and any(isinstance(source, dict) and source.get("type") == "official" for source in record.get("sources", []))}
    if CORE_PROVENANCE_FILES - core_files:
        problems.append("[provenance] missing official core dataset records: " + ", ".join(sorted(CORE_PROVENANCE_FILES - core_files)))
    catalog_ids = {record.get("entityId") for record in valid_records if record.get("entityKind") == "catalog-snapshot" and any(isinstance(source, dict) and source.get("type") == "official" for source in record.get("sources", []))}
    if CATALOG_PROVENANCE_IDS - catalog_ids:
        problems.append("[provenance] missing official catalog snapshots: " + ", ".join(sorted(CATALOG_PROVENANCE_IDS - catalog_ids)))

def validate():
    problems, domain_rows = [], {}
    for domain, config in CSV_CONFIG.items():
        _, rows = _check_file(f"domain:{domain}", DATA_DIR / config["file"], config["search_cols"], config["output_cols"], problems) or ([], [])
        domain_rows[domain] = rows
    for stack, config in STACK_CONFIG.items():
        headers, rows = _check_file(f"stack:{stack}", DATA_DIR / config["file"], _STACK_COLS["search_cols"], _STACK_COLS["output_cols"], problems) or ([], [])
    ...
    if reasoning_path.exists():
        _, reasoning = _check_file("reasoning", reasoning_path, ["UI_Category"], ["UI_Category", "Decision_Rules", "Reasoning", "Confidence"], problems) or ([], [])
    else:
    ...
    for row in landing:
        ...
        for identity in identities:
            ...
            if not folded:
                ...
                problems.append(f"[landing] ambiguous identity '{identity}' -> {owner}, {row.get('Pattern ID')}")
            else:
                landing_identities[folded] = row.get("Pattern ID")
    style_ids = _check_style_contract(styles, products, reasoning, problems, {"landing": pattern_ids, "style": {row.get("Style ID", "") for row in styles}})
